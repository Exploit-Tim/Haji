...
from Haji import dB, haji
from Haji.database import state
from Haji.helpers import (CMD, Emoji, Spotify, Tools, YoutubeSearch,
                          gen_qthumb, stream, telegram, youtube)
from Haji.logger import logger

__MODULES__ = "Streaming"
__HELP__ = """
<blockquote>⪼ **--Command help Streaming--**</blockquote>

<blockquote>**Basic command**</blockquote>
**ᐉ Keterangan: Gunakan `v` untuk memutar video**
`{0}play` (title)
        `{0}vplay` (title)
**ᐉ Keterangan: Resume playing**
`{0}resume`
**ᐉ Keterangan: Pause playing**
`{0}pause`
**ᐉ Keterangan: Skip playing**
`{0}skip`
**ᐉ Keterangan: End playing**
`{0}end`
        
<blockquote>**Channelplay command**</blockquote>
**ᐉ Keterangan: Play ke channel, gunakan `v` untuk memutar video**
ᐈ Perintah: `{0}cplay` (title) 
ᐈ Perintah: `{0}cvplay` title
**ᐉ Keterangan: Resume playing channel**
ᐈ Perintah: `{0}cresume`
**ᐉ Keterangan: Pause playing channel**
ᐈ Perintah: `{0}cpause`
**ᐉ Keterangan: Skip playing channel**
ᐈ Perintah: `{0}cskip`
**ᐉ Keterangan: End playing channel**
ᐈ Perintah: `{0}cend`
**ᐉ Keterangan: Linked channel to chat**
ᐈ Perintah: `{0}channelplay linked`
**ᐉ Keterangan: Disable linked channel**
ᐈ Perintah: `{0}channelplay disable`
**ᐉ Keterangan: Check linked playback channel**
ᐈ Perintah: `{0}channelplay status`
    
<blockquote>**Other command**</blockquote>
**ᐉ Keterangan: Atur volume pemutaran, lalu coba tinggalkan obrolan suara dan bergabung lagi**
ᐈ Perintah: `{0}volume` (1-200)
**ᐉ Keterangan: Dapatkan daftar putar yang diputar sekarang**
ᐈ Perintah: `{0}playlist`
    ...
    if str(query) == "disable":
        ...
        if chat and chat.linked_chat:
            ...
            if len(message.comand) < 3:
                return await message.reply(
                    f"{emo.gagal}**Please user command `{message.text.split()[0]} linked` @gcsupportmek"
                )
