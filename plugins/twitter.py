__MODULES__ = "Twitdl"
__HELP__ = """
<blockquote>⪼ **--Command help Twitter Download--**</blockquote>

<blockquote>**Download media from twitter**</blockquote>
**ᐉ Keterangan: Anda dapat mengunduh media apa pun dari Twitter**
ᐈ Perintah: `{0}twdl` (url)
...
async def _(client, message):
    ...
    if not arg:
        return await message.reply(
            f"{em.gagal}<b>Please give word or links!!\nExample: `{message.text.split()[0]} https://x.com/pakhajicabul/status/1887380401435648064/photo/1`</b>"
        )
    ...
    if not arg.startswith("https://"):
        return await proses.edit(
            f"{em.gagal}**Please provide link.\nExample: `{message.text.split()[0]} https://x.com/pakhajicabul/status/1887380401435648064/photo/1`**"
        )
