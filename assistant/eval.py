...
from Haji.database import DB_PATH, dB, state
from Haji.helpers import (CMD, ButtonUtils, Emoji, Message, Quotly, Sticker,
...
async def _(client, message):
    return await bot.send_message(
        ...
        reply_markup=ButtonUtils.userbot(haji._ubot[0].me.id, 0),
    )
...
async def restore(client, message):
    ...
    if not reply:
        return await message.reply("**ʜᴀʀᴀᴘ ᴍᴇᴍʙᴀʟᴀꜱ ᴋᴇ ꜰɪʟᴇ .db ᴀᴛᴀᴜ .zip**")
    ...
    if file_path.endswith(".zip"):
        ...
        if not db_files:
            return await message.reply("**ᴛɪᴅᴀᴋ ᴀᴅᴀ ꜰɪʟᴇ .db ʏᴀɴɢ ᴅɪᴛᴇᴍᴜᴋᴀɴ ᴅɪ ᴀʀꜱɪᴘ ZIP**")
        ...
        file_path = await client.download_media(document, "./")
    await message.reply(
        f"<blockquote><b>ꜱᴜᴋꜱᴇꜱ ᴍᴇʟᴀᴋᴜᴋᴀɴ ʀᴇꜱᴛᴏʀᴇ ᴅᴀᴛᴀʙᴀꜱᴇ, ᴛᴜɴɢɢᴜ ꜱᴇʙᴇɴᴛᴀʀ ʙᴏᴛ ᴀᴋᴀɴ ᴍᴇ-ʀᴇꜱᴛᴀʀᴛ.</blockquote></b>"
    )
